...
(defun strkm-books-export-csv (filename output nlines)
  ...
  (with-temp-buffer
    ...
    (let ((headers (mapconcat #'car strkm-books-columns strkm-csv-sep)))
      (insert headers "\n"))
    ;; Sostituisci i ritorni a capo con spazi nei dati per evitare problemi CSV
    (let ((clean-output (replace-regexp-in-string "\n" " " output)))
      (insert clean-output))
    ;; Salva il file
    (write-file filename)
...
(defun strkm-books-export-org (filename output nlines)
  ...
  (with-temp-buffer
    ...
    ;; Converti output in formato tabella org-mode, sostituendo newline con spazi
